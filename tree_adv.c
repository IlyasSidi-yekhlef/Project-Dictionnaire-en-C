...
p_tree_adv fill_tree_adv(){
    p_tree_adv simple_tree;
    for(int k = 0;k<=26;k++){
        simple_tree.root[k] = NULL;
    }
    int i;
    p_adv_node temp = NULL;
    int count = 1;
    char end = '\0';
    ...
    if (dicofile != NULL)
    {
        ...
        {
            if(formes[2] == 'v' && base[0]<='z'&& base[0] >= 'a' ){
                i = base[0] - 97;
                printf("base : %s\n",base);
                printf("emplacement : %d\n",i);
                if( simple_tree.root[i] == NULL && base[0] != end){
                    simple_tree.root[i] = creat_node_adv(base[0]);
                }
                ...
                while (base[count] != end) {
                    ...
                    if (temp->sons[i] == NULL && base[count] != end) {
                        printf("/OK/");
                        temp->sons[i] = creat_node_adv(base[count]);
                    }
                    temp = temp->sons[i];
                    count++;
                    printf("/%d\n",count);
